...
files_in_dayTIW = [f[:-6] + 'dayTIW.nc' for f in files_in_day]
files_in_mon = [f[:-6] + 'mon.nc' for f in files_in_day]
files_in_grd = [f[:-6] + 'grd.nc' for f in files_in_day]
files_out = [f[:-6] + 'mon_TIWhp.nc' for f in files_in]
...
for i,f_in in enumerate(files_in_day):
    ...
    ptools.calc_zhp_std_variables(f_in_dayTIW,f_in,f_in_mon,f_in_grd,f_out,filt_width)
